import{$ as t}from"./jquery-BvxTx_lq.js";t.ajaxSetup({headers:{"X-CSRF-TOKEN":t('meta[name="csrf-token"]').attr("content")}});let r=1;const b=10;function x(i){if(!i)return"-";const a=["Januari","Februari","Maret","April","Mei","Juni","Juli","Agustus","September","Oktober","November","Desember"],e=new Date(i);return isNaN(e)?"-":`${e.getDate()} ${a[e.getMonth()]} ${e.getFullYear()}`}function h(i){if(!i)return"";let e=i.toString().replace(/[^,\d]/g,"").split(","),n=e[0].length%3,s=e[0].substr(0,n),o=e[0].substr(n).match(/\d{3}/gi);return o&&(s+=(n?".":"")+o.join(".")),s=e[1]!==void 0?s+","+e[1]:s,"Rp "+s}function w(i){const a=t("#tableBody");if(a.empty(),i.length===0){a.append(`
            <tr>
        ...
        `);return}i.forEach((e,n)=>{let s=e.user?.status_user;s==="Verifikasi"&&(s="Verifikasi PBS");const o=s==="Verifikasi"?"bg-green-100 text-green-800":"bg-blue-100 text-blue-800",c=`
        ...
        `;a.append(c)})}function y(i){const a=t("#cardContainer");if(a.empty(),i.length===0){a.append(`
            <div class="p-6 text-center text-gray-500">
                <i class="fas fa-info-circle text-gray-400 mr-2"></i>
                Tidak ada data ditemukan
            </div>
        `);return}i.forEach(e=>{let n=e.user?.status_user;n==="Verifikasi"&&(n="Verifikasi PBS");const s=n==="Verifikasi"?"bg-green-100 text-green-800":"bg-blue-100 text-blue-800",o=`
        ...
        `;a.append(o)})}function d(i="",a=1){t.ajax({url:"/admin/getpendaftaranpbs",type:"GET",data:{search:i,page:a,limit:b},dataType:"json",success:function(e){const n=e.data;if(!Array.isArray(n)){console.error("Response data bukan array:",n);return}w(n),y(n),$(e.last_page,i),M(e.last_page,i);let s=(e.current_page-1)*b+1,o=s+n.length-1;t("#resultCount").html(`
                <i class="fas fa-info-circle mr-1"></i>
                Menampilkan ${s} - ${o} dari ${e.total} data
            `)},error:function(e,n,s){console.error("Gagal ambil data:",s,e.responseText)}})}function $(i,a){const e=t("#pagination");if(e.empty(),!(i<=1))for(let n=1;n<=i;n++){const s=t(`<button class="page-btn mx-1 px-3 py-1 rounded-lg border ${n===r?"bg-orange-primary text-white":"bg-white text-gray-700 hover:bg-gray-100"}">${n}</button>`);s.on("click",function(){r=n,d(a,r)}),e.append(s)}}function M(i,a){const e=t("#paginationMobile");if(e.empty(),!(i<=1))for(let n=1;n<=i;n++){const s=t(`<button class="px-3 py-1 rounded-lg border ${n===r?"bg-orange-primary text-white":"bg-white text-gray-700 hover:bg-gray-100"}">${n}</button>`);s.on("click",function(){r=n,d(a,r)}),e.append(s)}}t("#searchInputuser").on("input",function(){const i=t(this).val();r=1,d(i,r)});t(function(){d()});function j(i){const a=t("#"+i);a.removeClass("hidden"),setTimeout(()=>{a.find(".modal-content").addClass("show")},10),t("body").addClass("overflow-hidden")}function f(i){const a=t("#"+i);a.find(".modal-content").removeClass("show"),setTimeout(()=>{a.addClass("hidden"),t("body").removeClass("overflow-hidden")},300)}t(document).keydown(function(i){i.keyCode===27&&(t("#employeeModal").hasClass("hidden")?t("#activeModal").hasClass("hidden")||f("activeModal"):f("employeeModal"))});function m(i,a="info"){const s=t(`
    ...
    `);t("#notificationWrapper").append(s),setTimeout(()=>{s.removeClass("translate-x-full opacity-0")},100);const o=setTimeout(()=>{s.addClass("translate-x-full opacity-0"),setTimeout(()=>s.remove(),300)},4e3);s.on("click",function(){clearTimeout(o),t(this).addClass("translate-x-full opacity-0"),setTimeout(()=>t(this).remove(),300)})}function v(){t(window).width()<640?t(".modal-content").addClass("mx-4 max-h-[95vh]"):t(".modal-content").removeClass("mx-4 max-h-[95vh]")}t(window).on("resize",function(){v()});v();function C(){t("body").css({overflow:"hidden","padding-right":"15px"})}function T(){t("body").css({overflow:"","padding-right":""})}function k(i){j(i),C()}function l(i){f(i),T()}t("#closeModal, #cancelBtn").on("click",function(){l("employeeModal")});t("#cancelActiveBtn").on("click",function(){l("activeModal")});t("#cancelMahasiswaBtn").on("click",function(){l("detailMahasiswaModal")});t(".modal-overlay").on("click",function(i){i.target===this&&(t(this).closest("#employeeModal").length?l("employeeModal"):t(this).closest("#activeModal").length?l("activeModal"):t(this).closest("#detailMahasiswaModal").length&&l("detailMahasiswaModal"))});t(document).on("click",".aktif-btn",function(){const i=t(this).data("id"),a=t(this).data("name");t("#activeUserId").val(i),t("#activeUserName").text(a),k("activeModal")});t(document).on("click","#confirmActiveBtn",function(){const i=t(this),a=i.html();i.html('<i class="fas fa-spinner fa-spin mr-2"></i>Menyimpan...').prop("disabled",!0);const e=t("#activeUserId").val();t.ajax({url:`/admin/confirmpbs/${e}`,type:"PUT",success:function(n){m(n.message,n.status),l("activeModal"),d()},error:function(n){let s=n.responseJSON&&n.responseJSON.message?n.responseJSON.message:"Gagal menyimpan data!";m(s,"error")},complete:function(){i.html(a).prop("disabled",!1)}})});t(document).on("click",".detail-btn",function(){const i=t(this).data("id");k("detailMahasiswaModal"),t.ajax({url:`/admin/mahasiswa/detail/${i}`,type:"GET",success:function(a){t("#detail_nama_mahasiswa").text(a.user.name),t("#detail_nim").text("NIM: "+a.nim),t("#det_jk").text(a.jenis_kelamin),t("#det_email").text(a.user.email),t("#det_agama").text(a.agama),t("#det_telp").text(a.no_wa),t("#det_alamatktp").text(a.alamat_ktp),t("#det_tempatlahir").text(a.tempat_lahir),t("#det_nik").text(a.nik),t("#det_statusperkawinan").text(a.status_perkawinan),t("#det_jumlahsaudara").text(a.jumlah_saudara),t("#det_anakke").text(a.anak_ke),t("#det_semester").text(a.user.akademik.semester),t("#det_ipk").text(a.user.akademik.ip_terakhir),t("#det_status").text(a.user.status_user),t("#det_ayah_nama").text(a.user.orangtua.nama_ayah),t("#det_ayah_kerja").text(a.user.orangtua.pekerjaan_ayah),t("#det_ibu_nama").text(a.user.orangtua.nama_ibu),t("#det_ibu_kerja").text(a.user.orangtua.pekerjaan_ibu),t("#det_ayah_pendidikan").text(a.user.orangtua.pendidikan_ayah),t("#det_ibu_pendidikan").text(a.user.orangtua.pendidikan_ibu),t("#det_tgllahir").text(x(a.tanggal_lahir)),t("#det_universitas").text(a.mitra.nama_mitra),t("#det_ayah_gaji").text(h(a.user.orangtua.penghasilan_ayah)),t("#det_ibu_gaji").text(h(a.user.orangtua.penghasilan_ibu)),t("#det_ortu_telp").text(a.user.orangtua.no_wa_ortu),t("#det_tanggungan").text(a.user.orangtua.jumlah_tanggungan);const e=a.hasilujian&&a.hasilujian.length>0?a.hasilujian[0]:null;t("#det_potensi_benar").text(e?e.jumlah_benar:"-"),t("#det_potensi_salah").text(e?e.jumlah_salah:"-"),t("#det_potensi_tanggal").text(x(e?e.tanggal:"-")),t("#det_potensi_status").text(e&&e.kategori_soal?e.kategori_soal.name:"-");const n=t("#detail_foto_profil"),s=window.defaultAvatar;if(n.off("error"),a.foto){const u=window.location.origin+"/"+a.foto;n.one("error",function(){t(this).attr("src",s)}).attr("src",u)}else n.attr("src",s);const o=t("#document-list");o.empty();const c=[{field:"scan_ktp",label:"Scan KTP"},{field:"scan_kartu_mahasiswa",label:"Kartu Mahasiswa"},{field:"scan_kk",label:"Kartu Keluarga"},{field:"transkrip_nilai",label:"Transkrip Nilai"},{field:"surat_keterangan_aktif",label:"Surat Aktif"},{field:"essay_motivasi",label:"Essay Motivasi"}],g=a.user.dokumen;g?c.forEach(u=>{const p=g[u.field];if(p){const _=`
                        ...
                        `;o.append(_)}}):o.append('<p class="text-gray-500 italic text-sm">Belum ada dokumen yang diunggah.</p>')},error:function(){m("Gagal mengambil data detail","error")}})});t(document).on("click",".tab-btn",function(){const i=t(this).data("tab");t(".tab-btn").removeClass("active-tab border-blue-700 text-blue-700").addClass("border-transparent text-gray-500"),t(this).addClass("active-tab border-blue-700 text-blue-700").removeClass("border-transparent text-gray-500"),t(".tab-content").addClass("hidden"),t("#"+i).removeClass("hidden")});t(document).on("click",".detail-btn",function(){t('.tab-btn[data-tab="tab-biodata"]').trigger("click")});t(document).on("click",".download-pdf-btn",function(){const i=t(this).data("id");i&&fetch(`/admin/mahasiswa/pdf/${i}`,{method:"GET"}).then(a=>a.blob()).then(a=>{const e=window.URL.createObjectURL(a),n=document.createElement("a");n.href=e,n.download=`Biodata_${i}.pdf`,document.body.appendChild(n),n.click(),n.remove(),window.URL.revokeObjectURL(e)}).catch(()=>alert("Gagal download PDF"))});
