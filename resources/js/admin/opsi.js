        ...
        const row = `
            <tr class="hover:bg-gray-50 transition-colors duration-200">
                    ...
                    <div class="flex items-center">
                        <div class="flex-shrink-0 h-10 w-10">
                            <div class="h-10 w-10 rounded-full bg-gradient-to-r gradient-bg to-blue-light flex items-center justify-center text-white font-semibold">
                                ${pilihan.teks.charAt(0)}
                            </div>
                        </div>
                        <div class="ml-4">
                            <div class="text-sm font-medium text-gray-900">${pilihan.teks}</div>
                        </div>
                    </div>
                </td>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm">
                    <button class="edit-btn px-3 py-1 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition-all mr-2" data-id="${pilihan.id}">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="delete-btn px-3 py-1 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-all" data-id="${pilihan.id}" data-name="${pilihan.teks}">
...
function loadData(query = '', page = 1) {
    let idsoal = $('#soal_id').val();
    $.ajax({
        url: "/admin/pilihan/getpilihan/" + idsoal,
        type: "GET",
...
function resetForm() {
    $('#pilihanForm')[0].reset();
    $('#pilihanForm input, #pilihanForm select, #pilihanForm textarea').removeClass('border-red-300 bg-red-50');
}
...
$('#closeModal, #cancelBtn').on('click', function() {
    hideModalEnhanced('pilihanModal');
});
...
$('.modal-overlay').on('click', function(e) {
    if (e.target === this) {
        if ($(this).closest('#pilihanModal').length) {
            hideModalEnhanced('pilihanModal');
...
$('#addSubsoalBtn').on('click', function() {
    ...
    $('#modalTitle').text('Tambah Pilihan Soal Baru');
    $('#modalIcon').removeClass('fa-edit').addClass('fa-list-ul');
    $('#submitText').text('Simpan Data');
    $('#submitIcon').removeClass('fa-edit').addClass('fa-save');
    
    showModalEnhanced('pilihanModal');
});
...
$('#pilihanForm').on('submit', function(e) {
    e.preventDefault();
    ...
    const pilihanId = $('#pilihanId').val();
    const soal_Id = $('#soal_id').val(); 
    ...
    const url = pilihanId ? `/admin/pertanyaan/pilihan/${pilihanId}` : '/admin/pertanyaan/pilihan';
    const method = pilihanId ? 'PUT' : 'POST';
...
$(document).on('click', '.edit-btn', function() {
    ...
    $('#modalTitle').text('Edit Pilihan Soal');
    $('#modalIcon').removeClass('fa-list-ul').addClass('fa-edit');
    $('#submitText').text('Update Data');
    $('#submitIcon').removeClass('fa-save').addClass('fa-edit');

    $.ajax({
        url: '/admin/getpilihan/' + currentSoalsoalId,
        type: 'GET',
...
$(document).on('click', '#confirmDeleteBtn', function() {
    ...
    const pilihanId = $('#deletepilihanId').val();

    $.ajax({
        url: `/admin/pertanyaan/pilihan/${pilihanId}`, 
        type: 'DELETE',
        success: function(response) {
