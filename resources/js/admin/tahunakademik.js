...
function renderTable(data) {
    const tableTahunakademik = $('#tableTahunakademik');
    tableTahunakademik.empty();

    if (data.length === 0) {
        tableTahunakademik.append(`
            <tr>
    ...
    data.forEach((tahunakademik, index) => {
        const row = `
                    ...
                    <div class="flex items-center">
                        <div class="flex-shrink-0 h-10 w-10">
                            <div class="h-10 w-10 rounded-full bg-gradient-to-r gradient-bg to-blue-light flex items-center justify-center text-white font-semibold">
                                ${tahunakademik.tahun_akademik.charAt(0)}
                            </div>
                        </div>
                        <div class="ml-4">
                            <div class="text-sm font-medium text-gray-900">${tahunakademik.tahun_akademik}</div>
                        </div>
                    </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm">
                    <button class="edit-btn px-3 py-1 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition-all mr-2" data-id="${tahunakademik.id}">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="delete-btn px-3 py-1 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-all" data-id="${tahunakademik.id}" data-name="${tahunakademik.tahun_akademik}">
                        <i class="fas fa-trash"></i>
                    </button>
                </td>
            </tr>
        `;
        tableTahunakademik.append(row);
    });
}

// Render mobile cards khusus tahunakademik (hanya name + aksi edit/hapus)
            ...
            <div class="p-4 border-b border-gray-200 hover:bg-gray-50 transition-colors duration-200">
                <div class="flex items-center space-x-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <h3 class="text-lg font-semibold text-gray-900 truncate mb-2">${tahunakademik.tahun_akademik}</h3>
                        <div class="flex mt-4 space-x-2">
                            <button class="edit-btn flex-1 px-3 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition-all" data-id="${tahunakademik.id}">
                                <i class="fas fa-edit"></i> Edit
                            </button>
                            <button class="delete-btn flex-1 px-3 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-all" data-id="${tahunakademik.id}" data-name="${tahunakademik.tahun_akademik}">
...
// search event
$('#searchInputtahunakademik').on('input', function() {
...
function resetForm() {
    ...
    $('#tahunakademikForm input, #tahunakademikForm select, #tahunakademikForm textarea').removeClass('border-red-300 bg-red-50');
}
...
$('#closeModal, #cancelBtn').on('click', function() {
    hideModalEnhanced('tahunakademikModal');
});
...
$('.modal-overlay').on('click', function(e) {
    if (e.target === this) {
        if ($(this).closest('#tahunakademikModal').length) {
            hideModalEnhanced('tahunakademikModal');
...
let currentTahunakademikId = null;
let isEditMode = false;

// Show add Tahunakademik modal
$('#addTahunakademikBtn').on('click', function() {
    isEditMode = false;
    currentTahunakademikId = null;
    resetForm();
    $('#tahunakademikId').val('');
    
    $('#modalTitle').text('Tambah Tahun Akademik Baru');
    $('#modalIcon').removeClass('fa-edit').addClass('fa-calendar');
    $('#submitText').text('Simpan Data');
    $('#submitIcon').removeClass('fa-edit').addClass('fa-save');
    
    showModalEnhanced('tahunakademikModal');
});
...
function validateForm() {
    let isValid = true;
    const requiredFields = ['tahunakademikName'];
...
$('#tahunakademikForm').on('submit', function(e) {
    ...
    const tahunakademikId = $('#tahunakademikId').val();

    const formData = {
        tahun_akademik: $('#tahunakademikName').val(),
    };

    // Tentukan URL dan method berdasarkan mode
    const url = tahunakademikId ? `/admin/tahunakademik/${tahunakademikId}` : '/admin/tahunakademik';
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            loadData($('#searchInputtahunakademik').val(), currentPage);
        },
        error: function(xhr) {
            submitBtn.html(originalText).prop('disabled', false);
...
$(document).on('click', '.edit-btn', function() {
    isEditMode = true;
    currentTahunakademikId = $(this).data('id');
    resetForm();
    ...
    $.ajax({
        url: '/admin/tahunakademik/' + currentTahunakademikId,
        type: 'GET',
        success: function(tahunakademik) {
            $('#tahunakademikId').val(tahunakademik.id);
            $('#tahunakademikName').val(tahunakademik.tahun_akademik);

            showModal('tahunakademikModal');
        },
        error: function(xhr) {
            console.error("Gagal ambil data:", xhr.responseText);
            alert('Gagal ambil data tahunakademik');
        }
    });
});
...
$(document).on('click', '.delete-btn', function() {
    ...
    $('#deletetahunakademikId').val(id); 
    $('#deleteTahunakademikName').text(name);
...
$(document).on('click', '#confirmDeleteBtn', function() {
    ...
    const tahunakademikId = $('#deletetahunakademikId').val();

    $.ajax({
        url: `/admin/tahunakademik/${tahunakademikId}`, 
        type: 'DELETE',
        success: function(response) {
