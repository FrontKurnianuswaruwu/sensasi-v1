...
function renderTable(data) {
    const tablePengurus = $('#tablePengurus');
    tablePengurus.empty();

    if (data.length === 0) {
        tablePengurus.append(`
            <tr>
        ...
        const row = `
            ...
            <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <div class="text-sm font-medium text-gray-900">${pengurus.nama}</div>
                    </div>
                </div>
            </td>
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm">
                <button class="edit-btn px-3 py-1 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition-all mr-2" data-id="${pengurus.id}">
                    <i class="fas fa-edit"></i>
                </button>
                <button class="delete-btn px-3 py-1 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-all" data-id="${pengurus.id}" data-name="${pengurus.nama}">
                    <i class="fas fa-trash"></i>
                </button>
            </td>
            </tr>
        `;
        tablePengurus.append(row);
    });
}
...
function renderCards(data) {
    ...
    data.forEach((pengurus) => {
        ...
        if (pengurus.jabatan) {
            const jabatan = pengurus.jabatan.toLowerCase();
        ...
        const card = `
            ...
            <div class="flex items-start space-x-3">
                ...
                <div class="flex items-center justify-between mb-2">
                    <h3 class="text-lg font-semibold text-gray-900 truncate">${pengurus.nama}</h3>
                </div>
                <div class="space-y-1 text-sm text-gray-600">
                    <div class="flex items-center">
                    <i class="fas ${jabatanIcon} w-4 mr-2 text-orange-primary"></i>
                    <span class="truncate">${pengurus.jabatan || '-'}</span>
                    </div>
                    <div class="flex mt-4 space-x-2">
                    <button class="edit-btn flex-1 px-3 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition-all" data-id="${pengurus.id}">
                        <i class="fas fa-edit"></i> Edit
                    </button>
                    <button class="delete-btn flex-1 px-3 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-all" data-id="${pengurus.id}" data-name="${pengurus.nama}">
...
// search event
$('#searchInputpengurus').on('input', function() {
...
function resetForm() {
    ...
    $('#pengurusForm input, #pengurusForm select, #pengurusForm textarea').removeClass('border-red-300 bg-red-50');

    $('#pengurusForm input, #pengurusForm select').each(function() {
...
$('#closeModal, #cancelBtn').on('click', function() {
    hideModalEnhanced('pengurusModal');
});
...
$('.modal-overlay').on('click', function(e) {
    if (e.target === this) {
        if ($(this).closest('#pengurusModal').length) {
            hideModalEnhanced('pengurusModal');
...
$("#pengurusFoto").on("change", function () {
    const input = this;
...
$("#dropzone").on("drop", function (e) {
    ...
    $("#pengurusFoto")[0].files = files;
    $("#pengurusFoto").trigger("change");
})
...
let pengurusEditor;
let currentPengurusId = null;
let isEditMode = false;
// Show add Pengurus modal
$('#addSubpengurusBtn').on('click', function() {
    isEditMode = false;
    currentPengurusId = null;
    resetForm();
    $('#pengurusId').val('');

    $('#modalTitle').text('Tambah Pengurus Baru');
    $('#modalIcon').removeClass().addClass('fas fa-users');
    $('#submitText').text('Simpan Data');
    $('#submitIcon').removeClass('fa-edit').addClass('fa-save');

    showModalEnhanced('pengurusModal');
    
    $('#pengurusIsparent').val('');
});

$(function () {
    ClassicEditor
    .create(document.querySelector('#pengurusBiodata'))
    .then(editor => {
        pengurusEditor = editor;
    ...
    $('#pengurusForm').on('submit', function (e) {
        ...
        const pengurusId = $('#pengurusId').val();

        const formData = new FormData();
        formData.append('nama', $('#pengurusNama').val());
        formData.append('jabatan', $('#pengurusJabatan').val());
        formData.append('biodata', pengurusEditor.getData());

        const fileInput = $('#pengurusFoto')[0];
        ...
        const url = pengurusId ? `/admin/pengurus/${pengurusId}` : '/admin/pengurus';
        const method = 'POST';
        if (pengurusId) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                loadData($('#searchInputpengurus').val(), currentPage);
            },
            error: function (xhr) {
                submitBtn.html(originalText).prop('disabled', false);
...
$(document).on('click', '.edit-btn', function() {
    isEditMode = true;
    currentPengurusId = $(this).data('id');
    resetForm();

    $('#modalTitle').text('Edit Data Pengurus');
    $('#modalIcon').removeClass().addClass('fas fa-users');
    $('#submitText').text('Update Data');
    $('#submitIcon').removeClass('fa-save').addClass('fa-edit');

    $.ajax({
        url: '/admin/pengurus/' + currentPengurusId,
        type: 'GET',
        success: function(pengurus) {
            $('#pengurusId').val(pengurus.id);
            $('#pengurusNama').val(pengurus.nama);
            $('#pengurusJabatan').val(pengurus.jabatan);
            if (pengurusEditor) {
                pengurusEditor.setData(pengurus.biodata || '');
            } else {
                $('#pengurusBiodata').val(pengurus.biodata);
            }
            if (pengurus.foto) {
                $('#preview').attr('src', '/' + pengurus.foto);
                $('#previewContainer').removeClass('hidden');
                $('#oldFoto').val(pengurus.foto);
            } else {
                $('#preview').attr('src', '');
                $('#previewContainer').addClass('hidden');
                $('#oldFoto').val('');
            ...
            showModal('pengurusModal');
        },
        error: function(xhr) {
            console.error("Gagal ambil data:", xhr.responseText);
            alert('Gagal ambil data pengurus');
        }
    });
});
...
$(document).on('click', '.delete-btn', function() {
    ...
    $('#deletepengurusId').val(id); 
    $('#deletePengurusName').text(name);
...
$(document).on('click', '#confirmDeleteBtn', function() {
    ...
    const pengurusd = $('#deletepengurusId').val();

    $.ajax({
        url: `/admin/pengurus/${pengurusd}`, 
        type: 'DELETE',
        success: function(response) {
            showNotification(response.message , response.status);
