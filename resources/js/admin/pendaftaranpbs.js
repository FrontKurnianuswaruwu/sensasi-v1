...
function renderTable(data) {
    ...
    data.forEach((employee, index) => {
        ...
        if (status === 'Verifikasi') {
            status = 'Verifikasi PBS';
        }
        const statusClass = status === 'Verifikasi'
...
function renderCards(data) {
    ...
    data.forEach((employee) => {
        // Ambil status dari user object
        let status = employee.user?.status_user;

        // Logika penamaan status sesuai renderTable
        if (status === 'Verifikasi') {
            status = 'Verifikasi PBS';
        }
...
$(document).on('click', '.detail-btn', function() {
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            const hasil = res.hasilujian && res.hasilujian.length > 0 ? res.hasilujian[0] : null;

            $('#det_potensi_benar').text(hasil ? hasil.jumlah_benar : '-');
            $('#det_potensi_salah').text(hasil ? hasil.jumlah_salah : '-');
            $('#det_potensi_tanggal').text(formatTanggal(hasil ? hasil.tanggal : '-'));
            $('#det_potensi_status').text(hasil && hasil.kategori_soal ? hasil.kategori_soal.name : '-');
