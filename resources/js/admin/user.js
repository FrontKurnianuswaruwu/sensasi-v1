import $ from 'jquery';
const canEdit = $('meta[name="permission-edit-user"]').attr('content') === '1';
const canDelete = $('meta[name="permission-delete-user"]').attr('content') === '1';
tailwind.config = {
    theme: {
        extend: {
            colors: {
        ...
        const row = `
            <tr class="hover:bg-gray-50 transition-colors duration-200">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm">
                    ${canEdit ? `
                        <button class="edit-btn px-3 py-1 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition-all mr-2" data-id="${employee.id}">
                            <i class="fas fa-edit"></i>
                        </button>
                    ` : ''}
                    ${canDelete ? `
                        <button class="delete-btn px-3 py-1 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-all" data-id="${employee.id}" data-name="${employee.name}">
...
$(document).keydown(function(e) {
    if (e.keyCode === 27) { // Escape key
        ...
        } else if (!$('#deleteModal').hasClass('hidden')) {
...
function loadMitra(selectedType = '') {
    $.get('/admin/getmitra/user', function(response) {
        const mitra = Array.isArray(response.data) ? response.data : [];
        $('#employeeMitra').empty();
        $('#employeeMitra').append('<option value="">Pilih Mitra</option>');

        mitra.forEach(function(mitra) {
            var selected = mitra.id == selectedType ? 'selected' : '';
            $('#employeeMitra').append(`<option value="${mitra.id}" ${selected}>${mitra.nama_mitra}</option>`);
        });
    }).fail(function() {
        alert('Gagal mengambil data mitra. Pastikan API berjalan dengan benar.');
    });
}
...
$('#addEmployeeBtn').on('click', function() {
    ...
    $('#employeeMitra').closest('.space-y-2').hide();
    loadRoles();
    loadMitra();
...
$('#employeeRole').on('change', function () {
    const roleId = $(this).val(); 

    const roleMunculMitra = [9, 19];

    if (roleMunculMitra.includes(parseInt(roleId))) {
        $('#employeeMitra').closest('.space-y-2').show();
        loadMitra();
    } else {
        $('#employeeMitra').closest('.space-y-2').hide();
        $('#employeeMitra').val('');
...
function toggleMitraField(roleId, mitraId = '') {
    const canShow = roleId == 9 || roleId == 19;
...
$(document).on('click', '.edit-btn', function() {
    ...
    $.ajax({
        ...
        success: function(employee) {
            ...
            // ambil mitraId dari biodata_mahasiswa
            const mitraId = employee?.mitra_id || '';

            // tampilkan / sembunyikan field mitra berdasarkan roleId
            toggleMitraField(roleId, mitraId);
