        ...
        const row = `
            <tr class="hover:bg-gray-50 transition-colors duration-200">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm">
                    <button class="edit-btn px-3 py-1 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition-all mr-2" data-id="${role.id}">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="delete-btn px-3 py-1 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-all" data-id="${role.id}" data-name="${role.name}">
                        <i class="fas fa-trash"></i>
                    </button>
                </td>
            </tr>
        `;
        tableRole.append(row);
    });
}
            ...
            <div class="p-4 border-b border-gray-200 hover:bg-gray-50 transition-colors duration-200">
                <div class="flex items-center space-x-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <h3 class="text-lg font-semibold text-gray-900 truncate mb-2">${role.name}</h3>
                        <div class="flex mt-4 space-x-2">
                            <button class="edit-btn flex-1 px-3 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition-all" data-id="${role.id}">
                                <i class="fas fa-edit"></i> Edit
                            </button>
                            <button class="delete-btn flex-1 px-3 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-all" data-id="${role.id}" data-name="${role.name}">
...
// search event
$('#searchInputrole').on('input', function() {
...
function resetForm() {
    $('#roleForm')[0].reset();
    $('#roleForm input, #roleForm select, #roleForm textarea').removeClass('border-red-300 bg-red-50');
}
...
$('.modal-overlay').on('click', function(e) {
    if (e.target === this) {
        if ($(this).closest('#roleModal').length) {
            hideModalEnhanced('roleModal');
...
$('#roleForm').on('submit', function(e) {
    ...
    const url = roleId ? `/admin/user/${roleId}` : '/admin/role';
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            loadData($('#searchInputrole').val(), currentPage);
        },
        error: function(xhr) {
            submitBtn.html(originalText).prop('disabled', false);
...
$(document).on('click', '.edit-btn', function() {
    ...
    $('#modalTitle').text('Edit Data Role');
