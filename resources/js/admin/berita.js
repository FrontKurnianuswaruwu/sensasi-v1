    ...
    data.forEach((berita, index) => {
        const row = `
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm">
                <button class="edit-btn px-3 py-1 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition-all mr-2" data-id="${berita.id}">
                    <i class="fas fa-edit"></i>
                </button>
                <button class="delete-btn px-3 py-1 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-all" data-id="${berita.id}" data-name="${berita.judul}">
                    <i class="fas fa-trash"></i>
                </button>
            </td>
            </tr>
        `;
        tableBerita.append(row);
    });
}
        ...
        const card = `
            ...
            <div class="flex items-start space-x-3">
                ...
                <div class="flex items-center justify-between mb-2">
                    <h3 class="text-lg font-semibold text-gray-900 truncate">${berita.judul}</h3>
                </div>
                <div class="space-y-1 text-sm text-gray-600">
                    <div class="flex items-center">
                    <i class="fas fa-tags w-4 mr-2 text-orange-primary"></i>
                    <span class="truncate">${berita.ketegori}</span>
                    </div>
                    <div class="flex items-center">
                    <i class="fas fa-calendar-alt w-4 mr-2 text-blue-500"></i>
                    <span class="truncate">${formatTanggal(berita.tanggal)}</span>
                    </div>
                    <div class="flex mt-4 space-x-2">
                    <button class="edit-btn flex-1 px-3 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition-all" data-id="${berita.id}">
                        <i class="fas fa-edit"></i> Edit
                    </button>
                    <button class="delete-btn flex-1 px-3 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-all" data-id="${berita.id}" data-name="${berita.judul}">
...
// search event
$('#searchInputberita').on('input', function() {
...
function resetForm() {
    $('#beritaForm')[0].reset();
    $('#beritaForm input, #beritaForm select, #beritaForm textarea').removeClass('border-red-300 bg-red-50');

    $('#beritaForm input, #beritaForm select').each(function() {
...
$('.modal-overlay').on('click', function(e) {
    if (e.target === this) {
        if ($(this).closest('#beritaModal').length) {
            hideModalEnhanced('beritaModal');
...
$("#dropzone").on("drop", function (e) {
    ...
    $("#beritaFoto")[0].files = files;
    $("#beritaFoto").trigger("change");
})
...
$("#removeFoto").on("click", function () {
    let beritaId = $("#beritaId").val();

    if (!beritaId) {
        showNotification("ID Berita tidak ditemukan.");
        return;
    }

    $.ajax({
        url: '/berita/deleteFoto', 
        type: 'POST',
        data: { id: beritaId },
...
// Show add Berita modal
$('#addSubberitaBtn').on('click', function() {
    isEditMode = false;
...
$(function () {
    ClassicEditor
    .create(document.querySelector('#beritaDeskripsi'))
    .then(editor => {
        beritaEditor = editor;
    ...
    $('#beritaForm').on('submit', function (e) {
        ...
        formData.append('judul', $('#beritaJudul').val());
        formData.append('kategori', $('#beritaKategori').val());
        formData.append('tanggal', $('#beritaTanggal').val());
        formData.append('deskripsi', beritaEditor.getData());

        const fileInput = $('#beritaFoto')[0];
        ...
        const url = beritaId ? `/admin/berita/${beritaId}` : '/admin/berita';
        const method = 'POST';
        if (beritaId) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                loadData($('#searchInputberita').val(), currentPage);
            },
            error: function (xhr) {
                submitBtn.html(originalText).prop('disabled', false);
...
$(document).on('click', '.edit-btn', function() {
    isEditMode = true;
    currentBeritaId = $(this).data('id');
    resetForm();

    $('#modalTitle').text('Edit Data Berita');
    ...
    $.ajax({
        url: '/admin/berita/' + currentBeritaId,
        type: 'GET',
        success: function(berita) {
            $('#beritaId').val(berita.id);
            $('#beritaJudul').val(berita.judul);
            $('#beritaKategori').val(berita.ketegori);
            $('#beritaTanggal').val(berita.tanggal);
            if (beritaEditor) {
                beritaEditor.setData(berita.deskripsi || '');
            } else {
                $('#beritaDeskripsi').val(berita.deskripsi);
            }
            if (berita.foto) {
                $('#preview').attr('src', '/' + berita.foto);
                $('#previewContainer').removeClass('hidden');
                $('#oldFoto').val(berita.foto);
            } else {
                $('#preview').attr('src', '');
                $('#previewContainer').addClass('hidden');
                $('#oldFoto').val('');
...
$(document).on('click', '.delete-btn', function() {
    ...
    $('#deleteberitaId').val(id); 
    $('#deleteBeritaName').text(name);
...
$(document).on('click', '#confirmDeleteBtn', function() {
    ...
    const beritad = $('#deleteberitaId').val();

    $.ajax({
        url: `/admin/berita/${beritad}`, 
        type: 'DELETE',
        success: function(response) {
            showNotification(response.message , response.status);
