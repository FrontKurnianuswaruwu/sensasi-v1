...
function renderTable(data) {
    ...
    data.forEach((mitra, index) => {
        const row = `
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                ${mitra.tahun_kerjasama}
            </td>
            <td class="px-6 py-4 whitespace-nowrap text-sm">
                <button class="edit-btn px-3 py-1 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition-all mr-2" data-id="${mitra.id}">
                    <i class="fas fa-edit"></i>
                </button>
                <button class="delete-btn px-3 py-1 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-all" data-id="${mitra.id}" data-name="${mitra.nama_mitra}">
                    <i class="fas fa-trash"></i>
                </button>
            </td>
            </tr>
        `;
        tableMitra.append(row);
    });
}
    ...
    data.forEach((mitra) => {
        const card = `
            ...
            <div class="flex items-center justify-between mb-2">
                <h3 class="text-lg font-semibold text-gray-900 truncate">${mitra.nama_mitra}</h3>
            </div>
            <div class="space-y-1 text-sm text-gray-600">
                <div class="flex items-center">
                <i class="fas fa-user-tie w-4 mr-2 text-orange-primary"></i>
                <span class="truncate">${mitra.nama_admin || '-'}</span>
                </div>
                <div class="flex items-center">
                <i class="fas fa-phone-alt w-4 mr-2 text-blue-500"></i>
                <span class="truncate">${mitra.kontak || '-'}</span>
                </div>
                <div class="flex mt-4 space-x-2">
                <button class="edit-btn flex-1 px-3 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition-all" data-id="${mitra.id}">
                <i class="fas fa-edit"></i> Edit
                </button>
                <button class="delete-btn flex-1 px-3 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-all" data-id="${mitra.id}" data-name="${mitra.nama_mitra}">
...
// search event
$('#searchInputmitra').on('input', function() {
...
function resetForm() {
    $('#mitraForm')[0].reset();
    $('#mitraForm input, #mitraForm select, #mitraForm textarea').removeClass('border-red-300 bg-red-50');

    $('#mitraForm input, #mitraForm select').each(function() {
        $(this).removeClass('border-red-300 bg-red-50');
        hideFieldError(this);
    });

    if (typeof mitraEditor !== 'undefined' && mitraEditor) {
        mitraEditor.setData('');
    } else {
        $('#mitraDescription').val(''); 
    }
...
$('.modal-overlay').on('click', function(e) {
    if (e.target === this) {
        if ($(this).closest('#mitraModal').length) {
            hideModalEnhanced('mitraModal');
...
$("#dropzone").on("drop", function (e) {
    ...
    $("#mitraFoto")[0].files = files;
    $("#mitraFoto").trigger("change");
})
...
$("#removeFoto").on("click", function () {
    let mitraId = $("#mitraId").val();

    if (!mitraId) {
        showNotification("ID Mitra tidak ditemukan.");
        return;
    }

    $.ajax({
        url: '/mitra/deleteFoto', 
        type: 'POST',
        data: { id: mitraId },
...
// Show add Mitra modal
$('#addSubmitraBtn').on('click', function() {
    isEditMode = false;
    currentMitraId = null;
    resetForm();
    $('#mitraId').val('');

    $('#modalTitle').text('Tambah Mitra Baru');
...
$(function () {
    ClassicEditor
        .create(document.querySelector('#mitraDescription'))
        .then(editor => {
            mitraEditor = editor;
    ...
    $('#mitraForm').on('submit', function (e) {
        ...
        formData.append('name', $('#mitraName').val());
        formData.append('nama_admin', $('#adminName').val());
        formData.append('logo_url', $('#mitraLogourl').val());
        formData.append('link_website', $('#mitraLinkwebsite').val());
        formData.append('tahun_kerjasama', $('#mitraTahunkerjasama').val());
        formData.append('kontak', $('#mitraKontak').val());

        formData.append('deskripsi', mitraEditor.getData());

        const fileInput = $('#mitraFoto')[0];
        ...
        const url = mitraId ? `/admin/mitra/${mitraId}` : '/admin/mitra';
        const method = 'POST';
        if (mitraId) {
...
// Show edit mitra modal
$(document).on('click', '.edit-btn', function() {
    isEditMode = true;
    currentMitraId = $(this).data('id');
    resetForm();
    ...
    $.ajax({
        url: '/admin/mitra/' + currentMitraId,
        type: 'GET',
        success: function(mitra) {
            $('#mitraId').val(mitra.id);
            $('#mitraName').val(mitra.nama_mitra);
            $('#adminName').val(mitra.nama_admin);   
            $('#mitraLogourl').val(mitra.logo_url);
            $('#mitraLinkwebsite').val(mitra.link_website);
            $('#mitraKontak').val(mitra.kontak);
            $('#mitraTahunkerjasama').val(mitra.tahun_kerjasama);

            if (mitraEditor) {
                mitraEditor.setData(mitra.deskripsi || '');
            } else {
                $('#mitraDescription').val(mitra.deskripsi);
            }

            if (mitra.logo_url) {
                $('#preview').attr('src', '/' + mitra.logo_url);
                $('#previewContainer').removeClass('hidden');
                $('#oldFoto').val(mitra.logo_url);
            } else {
                $('#preview').attr('src', '');
                $('#previewContainer').addClass('hidden');
                $('#oldFoto').val('');
...
$(document).on('click', '#confirmDeleteBtn', function() {
    ...
    const mitraId = $('#deletemitraId').val();

    $.ajax({
        url: `/admin/mitra/${mitraId}`, 
        type: 'DELETE',
        success: function(response) {
            showNotification(response.message , response.status);
