...
function renderTable(data, isMahasiswa) {
    ...
    data.forEach((pengajuandana, index) => {
        ...
        const statusBadge = `<span class="px-3 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-700 border border-green-300">Approved</span>`;

        const namaMahasiswa = pengajuandana.mahasiswa?.user?.name ?? '-';
        const avatarChar = namaMahasiswa.charAt(0).toUpperCase();
        const universitas = pengajuandana.mahasiswa?.mitra?.nama_mitra ?? '-';
        const row = `
            <tr class="hover:bg-gray-50 transition-colors duration-200 border-b border-gray-100">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-sm text-gray-700 font-medium">${universitas}</span>
                    </div>
                </td>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-green-700 font-medium">
                    ${formatRupiah(pengajuandana.nominal_disetujui)}
                </td>
